...
MetronicApp.controller('StatCorporelsCtrl', function ($scope,
                                                  $stateParams,
                                                  $location,
                                                  $timeout,
                                                  DTOptionsBuilder,
    ...
    AccidentService.StatAccidentFilter().get({filter:'corporels'}, function (res) {
        var result = res.data;
        stat(null, result);
    });
    function stat(err, result) {
        ...
        dimensions.trimestre = data.dimension(function (record) {
            return record.trimestre;
        });
        dimensions.semestre = data.dimension(function (record) {
        ...
        charts.trimestre = dc.rowChart("#trimestre-selector");
        charts.trimestre.height(dimensions.trimestre.group().size() * rowChartHeight+40)
            .dimension(dimensions.trimestre)
            .group(dimensions.trimestre.group())
            .elasticX(true)
            .title(function (d) {
        ...
        charts.semestre = dc.rowChart("#semestre-selector");
        charts.semestre.height(dimensions.semestre.group().size() * rowChartHeight+40)
            .dimension(dimensions.semestre)
            .group(dimensions.semestre.group())
            .elasticX(true)
            .title(function (d) {
                return "" + d.key + ": " + d.value;
            })
